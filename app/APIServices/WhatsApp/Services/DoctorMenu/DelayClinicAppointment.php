...
class DelayClinicAppointment
{
    public static function execute(WhatsAppConversation $conversation, int $hours)
    {
        ...
        if (!$day) {
            ...
            $newStartTime = Carbon::parse($day->start_time)->addHours($hours)->format('H:i:s');
            $newEndTime = Carbon::parse($day->end_time)->addHours($hours)->format('H:i:s');
            ...
            $messageText = "تم تأخير موعد العيادة اليوم {$arabicHours} لتبدأ من {$startTime}\n" .
